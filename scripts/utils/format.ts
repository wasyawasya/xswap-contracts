...
const toDecimal = (wei: BigNumber, baseDecimals: number, toDecimals: number): string => {
  const [integer, fraction, fractionZeros] = toDecimalParts(wei, baseDecimals, toDecimals);
  const fractionPrefix = '0'.repeat(fractionZeros);
  const decimalStr = `${integer.toString()}.${fractionPrefix}${fraction.toString()}`;
  return decimalStr;
};

const formatEthString = (wei: BigNumber, decimals: number): string => {
